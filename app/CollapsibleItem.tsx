...
const CollapsibleItem: React.FC<CollapsibleItemProps> = ({
  ...
  function Jisho(subItem: string): { kanji: string; reading: string; meaning: string }[] {
    function KanjiReadingMeaningSplit(item: string) {
        const kanji = item.split("[")[0];
        const reading = item.split("[")[1].split("・")[0];
        const meaning = item.split("[")[1].split("・")[1].replace("]", "").trim();
  ...
  return (
    <div className='flex collapsibleItem'>
      <div className="border p-2 my-1 w-full">
        <div className="head-text cursor-pointer font-bold text-lg" onClick={toggleOpen}>
          {head}
        </div>
        {isOpen && (
                ...
                {index === 2 ? (
                  <div className="jisho-output-container bg-gray-50 p-2 rounded">
                    {Jisho(subItem).map((word, wordIndex) => (
                      <div key={wordIndex} className="jisho-word-entry mb-1 last:mb-0">
                        <ruby>
