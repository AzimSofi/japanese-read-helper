...
function cleanText(text) {
  // Remove all ruby tags first
  let cleaned = text.replace(/<ruby>.*?<\/ruby>/g, (match) => {
    // Extract just the base text (kanji) from the ruby tag
    const baseMatch = match.match(/<ruby>(.*?)<rt>.*?<\/rt><\/ruby>/);
    return baseMatch ? baseMatch[1] : match;
  });
